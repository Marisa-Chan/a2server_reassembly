...
Inputs:
  - migration_state/known_mfc.txt: ASM symbol names we do NOT migrate.

Outputs:
  - migration_state/to_migrate.txt: ASM symbol names we plan to migrate to C++.
  - migration_state/complete.txt: ASM symbol names already migrated to C++.

The script reads proc and data declarations from src/mfc_export.inc, then resolves
which .obj owns each symbol by parsing `dumpbin /symbols` output. Data symbols
(e.g. `externdef ?g_ShutdownIn@@3HA :dword`) are tracked the same way as procs:
owned by main.obj means still defined in ASM, any other .obj means migrated to C++.
"""
...
# MASM data type qualifiers accepted for data tracking (e.g. `:dword`, `:byte`).
# Untyped `public` lines (MFC exports) stay excluded, as before.
DATA_TYPES = {
    "byte", "sbyte", "word", "sword", "dword", "sdword", "qword", "sqword",
    "fword", "tbyte", "oword", "real4", "real8",
}
...
def parse_export_symbols(inc_path: Path) -> List[ExportSymbol]:
    ...
    with inc_path.open("r", encoding="utf-8", errors="replace") as f:
        for lineno, line in enumerate(f, 1):
            ...
            # Keep function declarations (`: proc`) and typed data declarations
            # (`: dword`, `: byte`, ...). Untyped `public` entries are skipped.
            if symbol_type != "proc" and symbol_type not in DATA_TYPES:
                continue
...
def collect_auto_to_migrate(
    ...
    """
    Build a to_migrate list from all resolved exports still in ASM.

        Rules:
        - include every parsed export (procs and data; legacy sub_/FUN_ names and
          already-mangled ones)
    ...
    for e in export_symbols:
        ...
        line_text = e.symbol
        # Spans only exist for procs (proc/endp); data symbols get no span column.
        span: Optional[int] = None
        if e.symbol_type == "proc":
            _, span = resolve_asm_span(e, asm_spans)
        if span is not None:
...
def collect_auto_complete(
    ...
    """
    Build complete list from all resolved exports owned by C++ .obj files.
...
def write_name_list_file(path: Path, header_note: str, asm_names: List[str]) -> None:
    lines = [
        ...
        "# Optional second tab-separated column stores ASM span (endp_line - proc_line, functions only).",
        f"# {header_note}",
        "",
    ]
...
def main() -> int:
    ...
    export_symbols = parse_export_symbols(inc_path)
    ...
    # Classify every parsed export (not just legacy sub_/FUN_-named ones), so the
    # totals below always add up to `len(export_symbols)`.
    seen_keys: Set[str] = set()
    migrated: List[Tuple[ExportSymbol, str]] = []
    still_asm: List[ExportSymbol] = []
    ...
    for e in export_symbols:
        ...
        if status == "cpp":
            ...
            migrated.append((e, owner))
        elif status == "asm":
            still_asm.append(e)
        else:
            unresolved.append((e.symbol, "declared but not defined in any .obj"))

    migrated.sort(key=lambda x: x[0].symbol.lower())
    still_asm.sort(key=lambda e: e.symbol.lower())
    unresolved.sort(key=lambda x: x[0].lower())

    def split_counts(entries: List[ExportSymbol]) -> Tuple[int, int]:
        procs = sum(1 for entry in entries if entry.symbol_type == "proc")
        return procs, len(entries) - procs

    parsed_procs, parsed_data = split_counts(export_symbols)
    mig_procs, mig_data = split_counts([entry for entry, _ in migrated])
    asm_procs, asm_data = split_counts(still_asm)

    print(f"Using dumpbin: {dumpbin_path}")
    print(f"Parsed exports  : {len(export_symbols)} ({parsed_procs} functions, {parsed_data} data)")
    ...
    for entry in still_asm:
        if entry.symbol_type != "proc":
            continue
    ...
    print("=" * 72)
    print(f"Migrated in C++ : {len(migrated)} ({mig_procs} functions, {mig_data} data)")
    print(f"Still in ASM    : {len(still_asm)} ({asm_procs} functions, {asm_data} data)")
    ...
    if overlap or dump_errors or missing_declared:
        ...
        if missing_declared:
            print(f"WARNING: {len(missing_declared)} symbols are declared in mfc_export.inc but not defined in any .obj")
